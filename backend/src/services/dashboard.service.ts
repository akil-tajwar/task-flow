...
export const dashboardService = {
  ...
  async get(tenantId: string, role: string): Promise<DashboardData> {
    ...
    const [todayRow] = await db.execute<{ count: string; total: string; profit: string }>(sql`
      SELECT
        COUNT(*)::text                     AS count,
    ...
    const [monthRow] = await db.execute<{ count: string; total: string; profit: string }>(sql`
      SELECT
        COUNT(*)::text                     AS count,
        COALESCE(SUM(total),0)::text       AS total,
        COALESCE(SUM(gross_profit),0)::text AS profit
    ...
    const catRows = await db.execute<{ name: string; total: string; profit: string; qty: string; cnt: string }>(sql`
      SELECT
        c.name,
    ...
    const recentRows = await db.execute<{ id: string; invoice_number: string; invoice_date: string; customer_name: string | null; total: string; status: string; payment_status: string }>(sql`
    ...
    const vsLastMonth   = lastMonthTotal > 0 ? ((monthTotal - lastMonthTotal) / lastMonthTotal) * 100 : 0;
    ...
    if (role === 'admin' || role === 'super_admin') {
      ...
      const [accountsData, trendRows, productRows, paymentRows, receivableRow, expenseRow] =
        await Promise.all([

          // Cash & bank accounts
          db.execute<{ id: string; name: string; category: string; current_balance: string }>(sql`
          ...
          // 14-day sales trend
          db.execute<{ date: string; total: string; profit: string; count: string }>(sql`
            SELECT
          ...
          db.execute<{ name: string; sku: string; qty: string; total: string; profit: string }>(sql`
            SELECT
              p.name,
      ...
      const totalCash  = cashAccts.reduce((s, a) => s + parseFloat(a.current_balance), 0);
      const totalBank  = bankAccts.reduce((s, a) => s + parseFloat(a.current_balance), 0);
