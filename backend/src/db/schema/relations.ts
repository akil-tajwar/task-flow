...
import { users, sessions, notifications } from './users';
import { roles, userRoles, skills, userSkills } from './roles';
...
import { invoices, invoiceLineItems, expenses, budgets, retainers } from './finance';
...
export const taskListsRelations = relations(taskLists, ({ one, many }) => ({
  project: one(projects, { fields: [taskLists.projectId], references: [projects.id] }),
  tasks: many(tasks),
}));
...
export const tasksRelations = relations(tasks, ({ one, many }) => ({
  ...
  taskList: one(taskLists, { fields: [tasks.taskListId], references: [taskLists.id] }),
...
export const invoiceLineItemsRelations = relations(invoiceLineItems, ({ one }) => ({
  ...
  task: one(tasks, { fields: [invoiceLineItems.taskId], references: [tasks.id] }),
...
export const retainersRelations = relations(retainers, ({ one }) => ({
  tenant: one(tenants, { fields: [retainers.tenantId], references: [tenants.id] }),
  client: one(clients, { fields: [retainers.clientId], references: [clients.id] }),
  project: one(projects, { fields: [retainers.projectId], references: [projects.id] }),
}));
...
export const ticketsRelations = relations(tickets, ({ one, many }) => ({
  ...
  inbox: one(deskInboxes, { fields: [tickets.inboxId], references: [deskInboxes.id] }),
...
export const deskInboxesRelations = relations(deskInboxes, ({ one, many }) => ({
  tenant: one(tenants, { fields: [deskInboxes.tenantId], references: [tenants.id] }),
  client: one(clients, { fields: [deskInboxes.clientId], references: [clients.id] }),
  tickets: many(tickets),
}));
...
export const spacePagesRelations = relations(spacePages, ({ one }) => ({
  ...
  author: one(users, { fields: [spacePages.createdBy], references: [users.id] }),
}));

export const proofsRelations = relations(proofs, ({ one, many }) => ({
  tenant: one(tenants, { fields: [proofs.tenantId], references: [tenants.id] }),
  project: one(projects, { fields: [proofs.projectId], references: [projects.id] }),
...
export const proofFeedbackRelations = relations(proofFeedback, ({ one }) => ({
  proof: one(proofs, { fields: [proofFeedback.proofId], references: [proofs.id] }),
  user: one(users, { fields: [proofFeedback.userId], references: [users.id] }),
}));

export const automationsRelations = relations(automations, ({ one, many }) => ({
  tenant: one(tenants, { fields: [automations.tenantId], references: [tenants.id] }),
