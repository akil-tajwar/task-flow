import { pgTable, uuid, varchar, text, boolean, timestamp, jsonb, integer, unique, index } from 'drizzle-orm/pg-core';
...
export const spacePageTags = pgTable(
  'space_page_tags',
  {
    pageId: uuid('page_id').notNull().references(() => spacePages.id, { onDelete: 'cascade' }),
